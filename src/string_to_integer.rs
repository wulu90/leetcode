...
impl Solution {
    pub fn my_atoi(str: String) -> i32 {
        ...
        while inx < len && bytes[inx] == 32_u8 {
            inx += 1;
        }

        if inx < len && bytes[inx] == 43_u8 {
            inx += 1;
        } else if inx < len && bytes[inx] == 45_u8 {
            sign = -1;
            inx += 1;
        }

        while inx < len && bytes[inx] >= 48_u8 && bytes[inx] <= 57_u8 {
            if i32::max_value() / 10 < res || (i32::max_value() / 10 == res && bytes[inx] > 55_u8) {
                return if sign == 1 {
                    i32::max_value()
                } else {
                    i32::min_value()
            ...
            res = res * 10 + bytes[inx] as i32 - 48;
            inx += 1;
        }
        res * sign
    }
